...
The frontend has a second build target that runs without the backend: `vite build --mode static` reads a JSON snapshot from `frontend/public/data` instead of `/api`. `.github/workflows/deploy-pages.yml` exports that snapshot straight from the database (the `DATABASE_URL` repository secret), builds, and publishes to `https://sanaan7788.github.io/local-business-finder/`. The repo is public, so everything in the snapshot is public.

**Publishing** is one workflow run, and every run refreshes the data:

- GitHub → Actions → "Deploy to GitHub Pages" → **Run workflow**, or
- `npm run publish:site` (uses the `gh` CLI).

Pushes to `main` that touch `frontend/` or `backend/src/` deploy as well. The snapshot files are generated by the workflow and gitignored; nothing about the data is committed.

Locally:

```bash
npm run export:static     # Neon → frontend/public/data; needs only DATABASE_URL; add -- --no-notes to leave CRM notes out
npm run build:static      # type-check + vite build --mode static + 404.html SPA fallback → frontend/dist
npm run preview:static    # serve the static build at http://localhost:4173/local-business-finder/
```

What the static site can do: browse, search, filter and sort businesses; open every profile tab; move leads between statuses (shortlist / reject / contacted / …) and write notes. Those edits live in that browser's localStorage (`lbf:changes`), merged over the snapshot on read. **Local changes** in the header exports them as a JSON file; apply it to the database and republish with:
...
frontend/
  src/
    ...
    ../public/data/         snapshot served by the static build (generated, gitignored)
